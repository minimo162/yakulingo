...
class TestIsTabularText:
    """Tests for is_tabular_text function"""

    def test_empty_string_is_not_tabular(self):
        """Empty string is not tabular"""
        assert is_tabular_text("") is False

    def test_none_is_not_tabular(self):
        """None is not tabular"""
        assert is_tabular_text(None) is False

    def test_plain_text_is_not_tabular(self):
        """Plain text without tabs is not tabular"""
        assert is_tabular_text("Hello world") is False
        assert is_tabular_text("こんにちは世界") is False

    def test_text_with_tab_is_tabular(self):
        """Text with tab character is tabular"""
        assert is_tabular_text("A\tB") is True
        assert is_tabular_text("売上\t利益") is True

    def test_multiline_tabular_text(self):
        """Multi-line tab-separated text is tabular"""
        text = "Header1\tHeader2\tHeader3\nValue1\tValue2\tValue3"
        assert is_tabular_text(text) is True

    def test_only_tabs_is_not_tabular(self):
        """Only tabs and whitespace is not tabular (no content)"""
        assert is_tabular_text("\t\t\t") is False
        assert is_tabular_text("\t\n\t") is False

    def test_excel_like_data(self):
        """Excel-like tabular data is detected"""
        excel_data = "項目\t金額\t備考\n売上\t100億円\t前年比+10%\n利益\t20億円\t"
        assert is_tabular_text(excel_data) is True

    def test_single_cell_with_tab(self):
        """Single cell followed by tab is tabular"""
        assert is_tabular_text("Value\t") is True


class TestTabularTemplates:
    """Tests for tabular text translation templates"""

    def test_tabular_templates_loaded(self):
        """Tabular templates are loaded from prompts directory"""
        # Using the actual prompts directory
        prompts_dir = Path(__file__).parent.parent / "prompts"
        if prompts_dir.exists():
            builder = PromptBuilder(prompts_dir=prompts_dir)
            # Check tabular templates are loaded
            assert "en" in builder._tabular_templates
            assert "jp" in builder._tabular_templates

    def test_get_tabular_template_en(self):
        """Get tabular template for English output"""
        prompts_dir = Path(__file__).parent.parent / "prompts"
        if prompts_dir.exists():
            builder = PromptBuilder(prompts_dir=prompts_dir)
            template = builder.get_tabular_template("en")
            assert template is not None
            assert "{input_text}" in template
            assert "{reference_section}" in template

    def test_get_tabular_template_jp(self):
        """Get tabular template for Japanese output"""
        prompts_dir = Path(__file__).parent.parent / "prompts"
        if prompts_dir.exists():
            builder = PromptBuilder(prompts_dir=prompts_dir)
            template = builder.get_tabular_template("jp")
            assert template is not None
            assert "{input_text}" in template
            assert "{reference_section}" in template

    def test_get_tabular_template_unknown_language(self):
        """Get tabular template for unknown language returns None"""
        builder = PromptBuilder(prompts_dir=None)
        template = builder.get_tabular_template("unknown")
        assert template is None

    def test_build_tabular_en(self):
        """Build tabular prompt for English output"""
        prompts_dir = Path(__file__).parent.parent / "prompts"
        if prompts_dir.exists():
            builder = PromptBuilder(prompts_dir=prompts_dir)
            input_text = "項目\t金額\n売上\t100億円"
            prompt = builder.build_tabular(input_text, output_language="en")
            assert prompt is not None
            assert input_text in prompt
            assert "tab" in prompt.lower() or "Tab" in prompt

    def test_build_tabular_jp(self):
        """Build tabular prompt for Japanese output"""
        prompts_dir = Path(__file__).parent.parent / "prompts"
        if prompts_dir.exists():
            builder = PromptBuilder(prompts_dir=prompts_dir)
            input_text = "Item\tAmount\nSales\t100 oku yen"
            prompt = builder.build_tabular(input_text, output_language="jp")
            assert prompt is not None
            assert input_text in prompt

    def test_build_tabular_with_reference(self):
        """Build tabular prompt with reference files"""
        prompts_dir = Path(__file__).parent.parent / "prompts"
        if prompts_dir.exists():
            builder = PromptBuilder(prompts_dir=prompts_dir)
            input_text = "Term\tValue"
            prompt = builder.build_tabular(input_text, has_reference_files=True, output_language="en")
            assert prompt is not None
            assert "Reference" in prompt or "参考" in prompt

    def test_build_tabular_no_template_returns_none(self):
        """Build tabular returns None when no template available"""
        builder = PromptBuilder(prompts_dir=None)  # No templates loaded
        prompt = builder.build_tabular("A\tB", output_language="en")
