...
> **Note**: ローカルAIの既定翻訳モデルは `mradermacher/DASD-4B-Thinking-GGUF/DASD-4B-Thinking.IQ4_XS.gguf` です。
> **Note**: ダウンロード先は `local_ai/models/DASD-4B-Thinking.IQ4_XS.gguf` です。
 ...
 ```json
   {
     ...
     "request_timeout": 600,
     "max_retries": 3,
    "local_ai_model_path": "local_ai/models/DASD-4B-Thinking.IQ4_XS.gguf",
...
**既定モデル（DASD-4B-Thinking）推奨値（ベースライン）**
```json
{
  "top_k": 64,
  "top_p": 0.95,
...
.\llama-cli.exe -m ..\..\models\DASD-4B-Thinking.IQ4_XS.gguf ^
...
| `local_ai_model_path` | ローカルAIモデル（.gguf）のパス | `local_ai/models/DASD-4B-Thinking.IQ4_XS.gguf` |
...
> **Note**: 既定モデル（`local_ai/models/DASD-4B-Thinking.IQ4_XS.gguf`）が存在しない場合、ローカルAIは起動しません。`packaging/install_deps.bat` を実行するか、`powershell -NoProfile -ExecutionPolicy Bypass -File packaging\\install_local_ai.ps1` を再実行してください。必要なら同名ファイルを手動で `local_ai/models/` に配置してください。
...
- モデルのダウンロードが失敗/404: ネットワーク/プロキシ設定を確認し `packaging/install_deps.bat` を再実行するか、`powershell -NoProfile -ExecutionPolicy Bypass -File packaging\\install_local_ai.ps1` を再実行してください。必要なら `mradermacher/DASD-4B-Thinking-GGUF` の `DASD-4B-Thinking.IQ4_XS.gguf` を `local_ai/models/DASD-4B-Thinking.IQ4_XS.gguf` に手動配置してください。
