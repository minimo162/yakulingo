...
def _queue_panel(
    ...
    pending = len(
        [i for i in state.file_queue if i.status == TranslationStatus.PENDING]
    )
    running = len(
        [i for i in state.file_queue if i.status == TranslationStatus.PROCESSING]
    )
    ...
    cancelled = len(
        [i for i in state.file_queue if i.status == TranslationStatus.CANCELLED]
    )
    ...
    with ui.element("div").classes("queue-panel"):
        with ui.row().classes(
            "queue-header items-center justify-between gap-2 flex-wrap"
        ):
        ...
        with ui.column().classes("queue-list w-full gap-2"):
            for idx, item in enumerate(state.file_queue):
                ...
                active_class = (
                    " active" if item.id == state.file_queue_active_id else ""
                )
                can_reorder = (
                    bool(on_reorder)
                    and not state.file_queue_running
                    and item.status == TranslationStatus.PENDING
                )
                with ui.element("div").classes(f"queue-item{active_class}") as row:
                    ...
                    with ui.column().classes("queue-item-body gap-1"):
                        ...
                        with ui.row().classes("items-center gap-2 flex-wrap"):
                            ui.label(item.status_label or item_status_label).classes(
                                status_class
                            )
                            if (
                                item.progress
                                and item.status == TranslationStatus.PROCESSING
                            ):
                                ui.label(f"{int(item.progress * 100)}%").classes(
                    ...
                    with ui.row().classes("queue-item-actions items-center gap-1"):
                        if (
                            ...
                            up_btn = ui.button(
                                icon="arrow_upward",
                                on_click=lambda i=item.id: on_move(i, -1),
                            )
                            up_btn.props(
