...
2. UIを開く（http://127.0.0.1:8765/）または `Ctrl + Alt + J` で翻訳を実行
...
| `Ctrl + Alt + J` (Windows) | 選択中のテキスト/ファイルを翻訳（グローバルホットキー、結果はクリップボードへ） |

**Ctrl + Alt + J の使い方**:
1. テキストの場合: 任意のアプリでテキストを選択 → `Ctrl + Alt + J` → 訳文がクリップボードへコピー
2. ファイルの場合: エクスプローラーでファイルを選択 → `Ctrl + Alt + J` → 翻訳済みファイルがクリップボードへコピー（フォルダで貼り付け）
   - 対応拡張子: `.xlsx` `.xls` `.docx` `.doc` `.pptx` `.ppt` `.pdf` `.txt`（最大10ファイルまで）
