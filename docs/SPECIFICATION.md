...
YakuLingoは「常駐＋ダブルコピー」を前提として動作します。
...
### 5.1.2 Ctrl+Cを短時間に2回（ダブルコピー）

Windows のクリップボード更新を監視し、同一ウィンドウで 700ms 以内に 2 回コピーされた場合に翻訳を実行する。
...
- 補足（統合）
  ...
  - Explorer右クリックメニューの `YakuLingoで翻訳` はこのAPIを利用する
...
- ダブルコピー（Ctrl+Cを短時間に2回）の起動動作は維持しつつ、UIのヒント行/キーキャップ表示を削除（ミニマル化）
