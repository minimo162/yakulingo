...
class PdfOperatorGenerator:
    ...
    def _encode_with_glyph_ids(self, font_id: str, text: str) -> str:
        ...
        if logger.isEnabledFor(logging.DEBUG):
            ...
            if missing_glyphs:
                logger.debug(
                    "Missing glyphs in font %s: %s", font_id, missing_glyphs[:10]
                )
...
class ContentStreamParser:
    ...
    def _filter_tokens(self, tokens: list[tuple[str, str]]) -> list[tuple[str, str]]:
        ...
        while i < n:
            ...
            if token_type == "operator":
                ...
                is_text_op = token_value in self.TEXT_OPS_TO_SKIP or (
        ...
        if operand_stack:
            logger.debug(
                "_filter_tokens: %d trailing operands at stream end", len(operand_stack)
            )
            result.extend(operand_stack)
    ...
    def filter_page_contents(self, doc, page) -> bytes:
        ...
        if not contents:
            logger.debug(
                "filter_page_contents: no content streams for page %d", page.number
            )
            return b""
...
class ContentStreamReplacer:
    ...
    def __init__(
        self, doc, font_registry: FontRegistry, preserve_graphics: bool = True
    ):
        self.doc = doc
        self.font_registry = font_registry
    ...
    def set_base_stream(
        ...
        if target_bboxes is not None:
            ...
            self._filtered_base_stream = self._parser.filter_page_contents(
                self.doc, page
            )

        return self

    def _filter_form_xobjects(
        self, page, allowed_xrefs: Optional[set[int]] = None
    ) -> None:
        """
        ...
        try:
            ...
            if not xobjects:
                logger.info(
                    "_filter_form_xobjects: no XObjects on page %d", page.number
                )
                return
            ...
            while xref_queue:
                ...
                try:
                    ...
                    if not stream:
                        logger.debug(
                            "_filter_form_xobjects: /%s (xref=%d) has no stream",
                            name,
                            xref,
                        )
                        continue
                    ...
                    self._find_nested_xobjects(
                        xref, obj_str, xref_queue, processed_xrefs
                    )

                except (RuntimeError, ValueError, KeyError, OSError) as e:
                    logger.debug(
                        "Could not filter Form XObject /%s (xref=%d): %s", name, xref, e
                    )
                    continue
            ...
            logger.debug(
                "Form XObject filtering skipped for page %d: %s", page.number, e
            )
        ...
        try:
            ...
            if resources_match:
                ...
                try:
                    ...
                    self._find_nested_xobjects(
                        resources_xref, resources_obj, xref_queue, processed_xrefs
                    )
                except (RuntimeError, ValueError, KeyError) as e:
                    logger.debug(
                        "Could not resolve Resources xref=%d: %s", resources_xref, e
                    )

        except (ValueError, AttributeError, re.error) as e:
            logger.debug(
                "Could not find nested XObjects in xref=%d: %s", parent_xref, e
            )
    ...
    def filter_all_document_xobjects(self) -> int:
        ...
        logger.info(
            "filter_all_document_xobjects: scanning %d xrefs in document", xref_count
        )
    ...
    def add_text_operator(
        self, op: str, font_id: str = None
    ) -> "ContentStreamReplacer":
        """
    ...
    def add_white_background(
        ...
        logger.debug(
            "add_white_background: x0=%.1f, y0=%.1f, x1=%.1f, y1=%.1f", x0, y0, x1, y1
        )
    ...
    def build_combined(self) -> bytes:
        ...
        if self._filtered_base_stream:
            ...
            filtered_str = self._filtered_base_stream.decode(
            ...
            logger.info(
                ...
                len(self._filtered_base_stream),
                len(combined),
    ...
    def apply_to_page(self, page) -> None:
        ...
        logger.debug(
            ...
            "filtered_base_len=%d",
            page.number,
            len(stream_bytes) if stream_bytes else 0,
            len(self.operators),
            list(self._used_fonts),
            len(self._filtered_base_stream) if self._filtered_base_stream else 0,
        )

        if not stream_bytes.strip():
            logger.warning(
                "apply_to_page: stream_bytes is empty, skipping page %d", page.number
            )
            return
    ...
    def _update_font_resources(self, page) -> None:
        ...
        for font_id in self._used_fonts:
            ...
            logger.debug(
                ...
                font_id,
                font_xref,
                list(self.font_registry._font_xrefs.keys()),
            )
            if font_xref:
        ...
        if font_dict_info[0] == "dict":
            ...
            existing = font_dict_info[
                1
            ].strip()  # Strip whitespace to fix endswith check
            ...
            existing_obj = self.doc.xref_object(
                font_xref_num
