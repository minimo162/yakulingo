...
**特徴:**
- **CSSズームのみ**: Pythonによるウィンドウサイズスケーリングは行わない
- **デュアルモニター対応**: `window.screen.width`はウィンドウが表示されているモニターの解像度を返すため、マルチモニター環境でも正しく動作
- **ウィンドウサイズ固定**: 設定ファイルの`window_width`/`window_height`（デフォルト: 1400×850）をそのまま使用

**動作例:**
| 解像度 | ズーム率 | ウィンドウサイズ |
|--------|----------|-----------------|
| 3840×2160 (4K) | 150% | 1400×850 |
| 2560×1440 (QHD) | 100% | 1400×850 |
| 1920×1080 (FHD) | 75% | 1400×850 |
| 1366×768 | 53% | 1400×850 |
...
- Pythonによるウィンドウスケーリングを削除し、CSSズームのみに統一
- デュアルモニター環境での表示問題を修正（`window.screen.width`で表示モニターを検出）
