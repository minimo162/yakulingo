...
| `Ctrl + Alt + J` (Windows) | 選択中のテキスト/ファイルを翻訳（グローバルホットキー。結果はUIに表示。テキストは必要な訳をコピー、ファイルはダウンロード） |

**Ctrl + Alt + J の使い方**:
1. テキストの場合: 任意のアプリでテキストを選択 → `Ctrl + Alt + J` → YakuLingo のUIに結果が表示（必要なスタイルをコピー）
2. ファイルの場合: エクスプローラーでファイルを選択 → `Ctrl + Alt + J` → UIのファイルタブに結果が表示（必要な出力をダウンロード）
...
3. YakuLingo のUIに結果が表示されます（必要な訳をコピー）
