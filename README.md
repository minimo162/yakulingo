...
2. **翻訳する** ボタンをクリック
3. 翻訳結果を確認
   - **日本語入力 → 英訳**: 標準/簡潔の2スタイルを表示（Copilot/ローカルAI共通）
...
- 英訳は2スタイル（標準/簡潔）から用途に合うものを選択（標準が最も丁寧）
...
- テキスト翻訳で次を入力して翻訳:
  ...
  経常損失は213億円となりました。
  ```
 - 期待: 標準/簡潔の2スタイルが表示され、いずれも「Revenue」だけ等の1語出力にならない

### 2スタイル欠落/遅延の手動QA（経営成績パラグラフ）

- サイドバー上部のCopilotボタンを **OFF（ローカルAI）/ON（Copilot）** それぞれで確認
- テキスト翻訳で次を入力して翻訳:
  ```text
  「経営成績」
  ...
  となりました。
  ```
 - 期待: 標準/簡潔の2スタイルが欠落せず表示される
 - 補足:
  - backendの応答でスタイルが欠けても、**不足分だけ再取得（最大1回）** → 揃わない場合は **フォールバック** で2スタイルを確定します
  - フォールバック時は、同じ訳文が複製される場合があります（最短で2スタイル表示を優先）
