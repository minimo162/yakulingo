...
@pytest.mark.parametrize(
    ("text", "expected"),
    [
        ("初任給は22万円です。", "初任給は220k yenです。"),
        ("設備投資は1万2千円です。", "設備投資は12k yenです。"),
        ("生産は15千台です。", "生産は15k unitsです。"),
        ("赤字は▲3千円です。", "赤字は(3k yen)です。"),
        ("▲1万2千円", "(12k yen)"),
    ],
)
...
def test_prompt_builder_pre_normalize_man_sen_does_not_break_cho_oku_compound() -> None:
    text = "総額は1億2万3千円です。"
    normalized = PromptBuilder.normalize_input_text(text, output_language="en")
    assert "billion" in normalized
    assert "2万3千円" in normalized
    assert "k" not in normalized
