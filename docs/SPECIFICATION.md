...
YakuLingoは「常駐＋ホットキー（Ctrl+Alt+J）」を前提として動作します。
...
### 5.1.2 ホットキー（Ctrl+Alt+J）

Windows のグローバルホットキー（Ctrl+Alt+J）を登録し、押下時に選択中のテキスト/ファイルをクリップボード経由で取得して翻訳を実行する。

- 実装概要
  - `RegisterHotKey`（`WM_HOTKEY`）で `Ctrl+Alt+J` を登録
  - トリガー時に前面ウィンドウのHWNDを取得（ウィンドウレイアウト用）
  - 必要に応じて `Ctrl+C` を送信し、選択内容をコピー → クリップボード内容を取得
  - クリップボード内容に応じてテキスト翻訳/ファイル翻訳を選択
    - `CF_UNICODETEXT`（テキスト）: 通常のテキスト翻訳ルートで処理（英訳は minimal のみ、和訳は訳文のみ / 解説なし / ストリーミング）→ UIに表示
...
**ローカルAI計測（task-00基準）:**
- warm を主指標、cold は参考
- 入力: `tools/bench_local_ai_input.txt`（400〜800字程度 / style=minimal）
...
- 互換: `--compare` を付ける（現行は英訳 minimal-only / options=1 / 追加呼び出しなし。`--input` 未指定時は `tools/bench_local_ai_input_short.txt`）
...
補足: 現行（ローカルAI）では、入力文にマッチした用語ペアのみを `### Glossary (matched; apply verbatim)` としてプロンプト内に差し込みます（上限あり）。
...
#### 9.4.2 英訳の出力言語ガード（正規化で不要リトライ削減）

背景:
- JP→EN で英訳結果に `兆/億/円` 等の日本語単位や全角記号が混入すると、「出力言語ミスマッチ→再試行」になりやすい。

対策:
- 出力言語ミスマッチ判定の前に、ガード用の最小正規化を行う（訳文の表示自体は自動変換しない）。
  - `unicodedata.normalize("NFKC", ...)`（全角→半角）
  - 日本語句読点の置換（例: `、`→`,`）
  - 和文金額の変換（例: `2兆2,385億円` → `22,385 oku yen`）
...
- 参照ファイルは `glossary.csv` のみを受け付ける。
  - ローカルAI: 添付できないため、入力文にマッチした用語ペアのみをプロンプトへ埋め込む（上限あり）。
...
- テキスト（minimal-only）:
  - 速度優先のため、単発テキスト翻訳では追加の再試行は行わない（best-effort）。
- バッチ（ファイル翻訳等）:
...
class AppSettings:
    ...
    local_ai_model_path: str = "local_ai/models/translategemma-4b-it.Q6_K.gguf"
...
> **Note**: ローカルAIの既定翻訳モデルは `local_ai/models/translategemma-4b-it.Q6_K.gguf`。使用するモデルは `local_ai_model_path`（または `local_ai_model_file`）で指定する。
...
### 2.22 (2026-01)
- 常駐/ホットキー
  - ダブルコピー（Ctrl+C x2）を廃止し、Ctrl+Alt+J のホットキー起動に変更
