...
- **Excel Translation Optimization (2024-12)**:
  ...
  - **Merged cells map optimization**: 結合セルマップをFindFormat APIで事前構築しキャッシュ
    - `_get_merged_cells_map()`: シートごとに1回のみ結合セルを一括取得
    - `_is_cell_in_merged_area()`: Python辞書検索で結合セル判定（COM呼び出しなし）
    - `_apply_font_batch_fast()`: 通常セルと結合セルを分離してバッチ処理
    - 無限ループ防止: `visited_addresses`セットと`MAX_ITERATIONS=10000`で安全性確保
    - パフォーマンス: バッチ失敗時の2000+ COM呼び出し → 数回に削減
