...
class TestVerifyChatInput:
    """Test _verify_chat_input() method"""

    def test_verify_chat_input_no_page(self):
        """_verify_chat_input returns False when no page"""
        handler = CopilotHandler()
        handler._page = None

        result = handler._verify_chat_input()
        assert result is False

    def test_verify_chat_input_success(self):
        """_verify_chat_input returns True when input is usable"""
        handler = CopilotHandler()

        mock_page = MagicMock()
        mock_input = MagicMock()
        mock_input.inner_text.return_value = "test"  # Input was received
        mock_page.wait_for_selector.return_value = mock_input
        handler._page = mock_page
        ...
        mock_input.fill.assert_called()  # Called twice: once with "test", once with ""

    def test_verify_chat_input_blocked(self):
        """_verify_chat_input returns False when input is blocked"""
        handler = CopilotHandler()

        mock_page = MagicMock()
        mock_input = MagicMock()
        mock_input.inner_text.return_value = ""  # Input was not received (blocked)
        mock_page.wait_for_selector.return_value = mock_input
        handler._page = mock_page
    ...
    def test_verify_chat_input_timeout(self):
        """_verify_chat_input returns False on timeout"""
        from playwright.sync_api import TimeoutError as PlaywrightTimeoutError
