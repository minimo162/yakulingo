...
class PromptBuilder:
    ...
    def build_concise_rewrite_prompt(
        self,
        input_text: str,
        *,
        output_language: str,
        ...
        if output_language == "jp":
            ...
            return (
                ...
                f"abbreviation（略語）を積極的に使用してください（例: 粗利=売上総利益、営利=営業利益、販管費=販売費及び一般管理費、前年差=前年同期間の差）。"
                f"必ず日本語（漢字/ひらがな/カタカナ）を含め、英語だけの出力は禁止です（略語の英字は可）。"
        ...
        return (
            ...
            f"Use abbreviations aggressively where appropriate (e.g., YoY, QoQ, FY, KPI, OP (operating profit), consol. (consolidated), incl., excl., w/, w/o). "
