...
if not exist "uv.exe" (
    ...
    if not exist "uv.exe" (
        ...
        call :prompt_proxy_auth
        if defined HTTP_PROXY (
            echo [INFO] Retrying with proxy authentication...
            powershell -ExecutionPolicy Bypass -Command ^
                "$ProgressPreference = 'SilentlyContinue'; " ^
...
:: ============================================================
:: Function: Prompt for proxy authentication
:: ============================================================
:prompt_proxy_auth
echo.
echo ============================================================
...
if not defined PROXY_SERVER (
    set /p PROXY_SERVER="Proxy server (e.g., proxy.example.com:8080): "
)
if not defined PROXY_SERVER (
    echo [SKIP] No proxy server specified.
    exit /b 0
)
