...
- 既定値は `local_ai_device=Vulkan0` / `local_ai_n_gpu_layers=99`。CPU-only に戻す場合は `none` / `0` を設定します。
...
| `local_ai_ctx_size` | ローカルAIのcontext size | 8192 |
...
| `local_ai_n_gpu_layers` | GPUに載せる層数（`0` / `99` / `auto` / `all`） | 99 |
