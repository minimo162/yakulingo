...
def test_local_text_options_streams_translation(monkeypatch) -> None:
    ...
    service = TranslationService(copilot=object(), config=settings, prompts_dir=Path("prompts"))
    received: list[str] = []

    def on_chunk(text: str) -> None:
        received.append(text)

    def fake_translate_single_with_cancel(text, prompt, reference_files=None, on_chunk=None):
        if on_chunk:
    ...
    monkeypatch.setattr(service, "_translate_single_with_cancel", fake_translate_single_with_cancel)
...
def test_local_text_style_comparison_streams_translation(monkeypatch) -> None:
    ...
    def fake_translate_single_with_cancel(text, prompt, reference_files=None, on_chunk=None):
        if on_chunk:
            on_chunk('{"output_language":"en","options":[{"style":"standard","translation":"H')
