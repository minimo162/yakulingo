...
class TestNetworkErrors:
    ...
    def test_network_disconnection_mid_batch(self, settings, large_excel):
        ...
        def simulate_disconnect(texts, prompt, ref=None, char_limit=None):
            call_count[0] += 1
            if call_count[0] > 1:
...
class TestPartialBatchFailures:
    ...
    def test_first_batch_fails_second_succeeds(self, settings):
        ...
        def batch_with_first_failure(texts, prompt, ref=None, char_limit=None):
            call_count[0] += 1
            if call_count[0] == 1:
...
class TestCancellationDuringErrors:
    ...
    def test_cancel_after_partial_completion(self, settings, large_excel):
        ...
        def translate_then_cancel(texts, prompt, ref=None, char_limit=None):
            call_count[0] += 1
