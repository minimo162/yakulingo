...
- テキスト翻訳で次を入力して翻訳:
  ...
  となりました。
  ```
- 期待: 標準/簡潔/最簡潔の3スタイルが欠落せず表示される
- 補足:
  - backendの応答でスタイルが欠けても、**不足分だけ再取得（最大1回）** → 揃わない場合は **フォールバック** で3スタイルを確定します
  - フォールバック時は、同じ訳文が複製される場合があります（最短で3スタイル表示を優先）
